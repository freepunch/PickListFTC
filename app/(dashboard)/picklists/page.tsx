...
export default function PickListsPage() {
  ...
  const [openingToast, setOpeningToast] = useState<string | null>(null);
  ...
  const handleOpenPickList = useCallback(
    (eventCode: string) => {
      const name = pickLists.find((pl) => pl.eventCode === eventCode)?.eventName ?? eventCode;
      setOpeningToast(name);
      setEventCode(eventCode);
      loadEvent(eventCode);
      router.push("/picklist");
    },
    [loadEvent, setEventCode, router, pickLists]
  );
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6">
      ...
      {/* Event loaded toast */}
      {openingToast && (
        <div className="fixed bottom-5 right-5 z-50 flex items-center gap-3 px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-700 shadow-xl animate-fade-in">
          ...
          <p className="text-sm text-zinc-200">
            Event <span className="font-medium text-white">{openingToast}</span> loaded
          </p>
        </div>
      )}
