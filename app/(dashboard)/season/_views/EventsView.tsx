...
function getLocalNotes(userId?: string | null): NoteSummary[] {
  ...
  for (const { key, eventCode } of findScopedKeys("notes", userId)) {
    try {
      ...
      notes.push({ eventCode, noteCount: parsed.length });
    } catch { continue; }
  }
  return notes;
}
      ...
      >
        <div className="sticky top-0 bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex items-start justify-between z-10">
          <div className="min-w-0 flex-1 mr-4">
            <h2 className="text-lg font-semibold text-white truncate">{event.event_name ?? event.event_code}</h2>
          ...
          <button onClick={onClose} className="p-1.5 text-zinc-600 hover:text-white hover:bg-zinc-800 rounded-md transition-colors shrink-0">
        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-2">
            <button onClick={onLoad} className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white rounded-lg transition-colors">
              Load Event
            </button>
            <Link href="/leaderboard" onClick={onLoad} className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg transition-colors border border-zinc-700">
              Leaderboard
            </Link>
          </div>
          ...
          <div className="bg-zinc-800/50 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
              ...
              {pickList && <span className="text-[10px] text-zinc-500">{pickList.teamCount} teams ranked</span>}
            </div>
            {preview.length > 0 ? (
              ...
              <div className="px-4 py-6 text-center"><p className="text-xs text-zinc-600">No pick list yet for this event</p></div>
            )}
            {pickList && (
              <div className="px-4 py-2 border-t border-zinc-800">
                <Link href="/picklist" onClick={onLoad} className="text-xs text-[var(--accent)] hover:underline">Open Full List</Link>
              </div>
            )}
          </div>
          ...
          <div className="bg-zinc-800/50 border border-zinc-800 rounded-xl overflow-hidden">
            <div className="px-4 py-3 border-b border-zinc-800 flex items-center justify-between">
              ...
              {noteCount > 0 && <span className="text-[10px] text-zinc-500">{noteCount} note{noteCount !== 1 ? "s" : ""}</span>}
            </div>
                  ...
                  <div key={i} className="px-4 py-2.5">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      {note.tags.slice(0, 2).map((tag) => (
                        <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full bg-zinc-700 text-zinc-400">{tag}</span>
                      ))}
                    </div>
                    {note.text && <p className="text-xs text-zinc-400 line-clamp-2">{note.text}</p>}
                  </div>
                ))}
              </div>
            ) : (
              <div className="px-4 py-6 text-center"><p className="text-xs text-zinc-600">No scouting notes yet for this event</p></div>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-fade-in w-full max-w-sm px-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl p-4 flex flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={onDismiss} className="text-zinc-600 hover:text-zinc-400 transition-colors shrink-0 mt-0.5" aria-label="Dismiss">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        </div>
        <button onClick={onGo} className="w-full flex items-center justify-center gap-2 py-2 text-sm font-medium bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white rounded-lg transition-colors">
          Go to Dashboard
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function EventsView() {
  ...
  useEffect(() => {
    setPickLists(getLocalPickLists(userId));
    setNotes(getLocalNotes(userId));
    setTopTeams(getTopTeams(userId));
  }, [userId]);

  const pickListMap = useMemo(() => new Map(pickLists.map((p) => [p.eventCode, p])), [pickLists]);
  const noteMap = useMemo(() => new Map(notes.map((n) => [n.eventCode, n.noteCount])), [notes]);
  ...
  const drawerEventData = drawerEvent ? favoriteEvents.find((e) => e.event_code === drawerEvent) : null;
    ...
    return (
      <>
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h2 className="text-lg font-semibold text-zinc-200 mb-2">Add events to start building your season dashboard</h2>
          ...
          <div className="w-full max-w-md"><EventLoader bare /></div>
          <p className="text-xs text-zinc-600 mt-4">After loading an event, click the star icon in the sidebar to watch it</p>
        </div>
        {eventToast && (
          <EventLoadedToast
      ...
      {nextEvent && nextEvent.start && (
        <div className="mb-8 bg-gradient-to-r from-[var(--accent)]/5 to-transparent border border-[var(--accent)]/20 rounded-xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h2 className="text-lg font-semibold text-white">{nextEvent.event_name ?? nextEvent.event_code}</h2>
            <p className="text-sm text-zinc-400 mt-0.5">
              {new Date(nextEvent.start).toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}
            </p>
            {(() => {
              const days = Math.ceil((new Date(nextEvent.start).getTime() - Date.now()) / 86400000);
              return (
                <p className="text-sm text-zinc-500 mt-1">
                  <span className="text-xl font-bold text-[var(--accent)]">{days > 0 ? days : 0}</span> day{days !== 1 ? "s" : ""} away
                </p>
              );
            })()}
          </div>
            ...
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1 text-[10px] font-medium rounded-md transition-colors ${
                  sortMode === s.key ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                {s.label}
              </button>
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white truncate group-hover:text-white">{ev.event_name ?? ev.event_code}</p>
                    {ev.start && (
                ...
                <div className="flex items-center gap-3 text-xs text-zinc-500">
                  ...
                  {!pl && nc === 0 && <span className="text-zinc-700">No scouting data</span>}
                </div>
              </div>
            );
          })}
          ...
          <div className="divide-y divide-zinc-800/50">
            {topTeams.map((t) => (
              ...
              >
                ...
                <span className="text-xs text-zinc-500">{t.count} event{t.count !== 1 ? "s" : ""}</span>
              </Link>
            ))}
          </div>
        </div>
