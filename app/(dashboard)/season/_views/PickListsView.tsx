  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
        <h3 className="text-sm font-semibold text-zinc-200">Comparing Pick Lists</h3>
                ...
                {list.entries.filter((e) => !e.picked).map((entry, i) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-2 px-4 py-1.5 text-xs border-b border-zinc-800/30 ${isCommon ? "bg-emerald-500/5" : ""}`}
                    >
                      <span className="text-zinc-600 font-mono w-6 text-right">{i + 1}</span>
                      <span className="font-mono text-white w-12 font-medium">{entry.teamNumber}</span>
                      <span className="text-zinc-400 flex-1 truncate">{entry.teamName}</span>
                      {isCommon && <span className="text-emerald-400 text-[10px] font-medium shrink-0">Common</span>}
                    </div>
                  );
                })}
              </div>
...
export function PickListsView() {
  ...
  const handleDelete = useCallback(
    async (eventCode: string) => {
      try { localStorage.removeItem(picklistKey(eventCode, userId)); } catch { /* ignore */ }
      if (user) {
        try {
          await supabase.from("pick_lists").delete().eq("user_id", user.id).eq("event_code", eventCode);
        } catch { /* ignore */ }
      }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const nameMap = new Map(favoriteEvents.map((e) => [e.event_code, e.event_name]));
      const lists = Array.from(localMap.values()).map((l) => ({
        ...l,
        eventName: nameMap.get(l.eventCode) ?? l.eventCode,
      }));
      lists.sort((a, b) => new Date(b.lastEdited).getTime() - new Date(a.lastEdited).getTime());
      setPickLists(lists);
      setLoading(false);
    };
    load();
  ...
  const handleToggleCompare = (eventCode: string) => {
    setCompareSelection((prev) => {
      if (prev.includes(eventCode)) return prev.filter((c) => c !== eventCode);
      if (prev.length >= 2) return [prev[1], eventCode];
    ...
    <>
      ...
      {compareMode && !compareData && (
        <div className="mb-4 bg-zinc-800/50 border border-zinc-700 rounded-lg px-4 py-3 flex items-center justify-between">
          <p className="text-xs text-zinc-400">
            Select 2 pick lists to compare. <span className="text-zinc-500">{compareSelection.length}/2 selected</span>
          </p>
      ...
      {!loading && pickLists.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <h2 className="text-lg font-semibold text-zinc-200 mb-2">No pick lists yet</h2>
          ...
          {pickLists.map((pl) => {
            ...
            return (
              <div
                key={pl.eventCode}
                onClick={() => {
                  if (compareMode) handleToggleCompare(pl.eventCode);
                  else handleOpenPickList(pl.eventCode);
                }}
              ...
              >
                  ...
                  <div className="absolute top-3 right-3">
                    <div
                      className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${
                        isSelected ? "bg-[var(--accent)] border-[var(--accent)]" : "border-zinc-700"
                      }`}
                    >
                      {isSelected && (
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-white truncate">{pl.eventName}</p>
                    <p className="text-xs text-[var(--accent)] font-mono mt-0.5">{pl.eventCode}</p>
                  </div>
                  {!compareMode && (
                    <button
                      onClick={(e) => {
                      ...
                      className="p-1 rounded-md text-zinc-600 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all shrink-0"
