...
import { TeamEventStats2025 } from "@/lib/types";
...
export function WatchedTeamsView() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled(
      ...
      setLoadedTeams((prev) => {
        ...
        for (const result of results) {
          if (result.status === "fulfilled") next.add(result.value.teamNumber);
        }
        return next;
      });
      setLoading(false);
  ...
  const sortedTeams = useMemo(() => {
    return favoriteTeams
      .map((fav) => ({ fav, data: teamData.get(fav.team_number) }))
      .sort((a, b) => (b.data?.latestOpr ?? 0) - (a.data?.latestOpr ?? 0));
  ...
  if (favoriteTeams.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-lg font-semibold text-zinc-200 mb-2">No watched teams yet</h2>
        <p className="text-sm text-zinc-500 max-w-sm">
          Star teams from the leaderboard or team reports to track them across all events.
        </p>
      </div>
    );
  }
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-zinc-800">
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Latest OPR</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider hidden sm:table-cell">Last Event</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Events</th>
              <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider hidden md:table-cell">Record</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Trend</th>
            ...
            {sortedTeams.map(({ fav, data }, i) => (
              ...
              >
                <td className="px-4 py-3">
                  <Link href={`/report/${fav.team_number}`} className="hover:underline">
                    <span className="font-mono text-white text-sm font-medium mr-2">{fav.team_number}</span>
                    <span className="text-zinc-400">{data?.teamName ?? fav.team_name ?? ""}</span>
                  </Link>
                </td>
                ...
                <td className="px-4 py-3 font-mono text-zinc-400 text-sm hidden md:table-cell">
                  {data ? `${data.record.wins}-${data.record.losses}-${data.record.ties}` : "—"}
                </td>
