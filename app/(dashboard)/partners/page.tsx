...
function SelectedTeamCard({ team }: { team: ProcessedTeam }) {
...
function ExpandedPanel({
  ...
  const diffs = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Total Avg",
        a: s.avg.totalPointsNp,
        b: p.avg.totalPointsNp,
      },
      {
        label: "Consistency",
        a: s.dev.totalPointsNp,
        b: p.dev.totalPointsNp,
        invert: true,
      },
    ];
  }, [selected, partner]);
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm font-semibold text-white w-8 text-right">
            {result.score}
          </span>
        </div>

        {/* Combined OPR */}
        <span className="font-mono text-sm text-zinc-300 text-center">
          {result.projectedCombinedOpr.toFixed(1)}
        </span>

        {/* Tag */}
        <span
          className={`text-xs font-medium px-2 py-0.5 rounded-full border whitespace-nowrap w-fit ${tagColor(result.complementarityTag)}`}
        >
...
export default function PartnersPage() {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {/* Selected team card */}
      {selectedTeam && <SelectedTeamCard team={selectedTeam} />}
        ...
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl overflow-hidden">
          ...
          {ranked.map((result, idx) => (
            <PartnerRow
              ...
              onToggle={() =>
                setExpandedRow(
                  expandedRow === result.teamNumber ? null : result.teamNumber
                )
              }
            />
          ))}
