...
function SidebarContent({
  ...
  return (
    <>
          ...
          {PLAN_NAV
            ...
            return (
              <div key={item.href} className="relative">
                ...
                {showDot && (
                  <span
                    className="absolute top-2 right-2 w-2 h-2 rounded-full bg-amber-400 border border-zinc-900"
      ...
      {/* Workspace indicator — only when the user is in a workspace */}
      {ws && (
        <Link
          href="/workspace"
          onClick={onNavClick}
          title={collapsed ? `Workspace: ${ws.name}` : undefined}
          className={`relative mx-2 mb-1 flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-colors border border-[var(--border)] bg-[var(--bg-card-hover)]/40 hover:bg-[var(--bg-card-hover)] text-[var(--foreground-muted)] ${collapsed ? "justify-center" : ""}`}
        >
          <svg className="w-4 h-4 shrink-0 text-[var(--accent)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
          </svg>
          {!collapsed && (
            <span className="flex-1 truncate text-[var(--foreground)]">{ws.name}</span>
          )}
          {workspacePendingCount > 0 && (
            <span
              className={`${collapsed ? "absolute top-1 right-1" : ""} w-2 h-2 rounded-full bg-amber-400`}
              title={`${workspacePendingCount} pending suggestion${workspacePendingCount === 1 ? "" : "s"}`}
            />
          )}
        </Link>
